...
async def cleanup_mcp_client():
    ...
    """
    # 문법 오류 수정: 'is not None' 조건을 한 줄로 결합
...
def get_streaming_callback(text_placeholder, tool_placeholder):
    ...
    반환값:
        ...
        accumulated_text: 누적된 텍스트 응답 리스트
        accumulated_tool: 누적된 도구 호출 정보 리스트
    """
    accumulated_text = []
    accumulated_tool = []
...
async def process_query(query, text_placeholder, tool_placeholder, timeout_seconds=60):
    ...
    반환값:
        response: 에이전트의 응답 객체 (에러 발생 시 {"error": "..."} 형식)
        final_text: 최종 텍스트 응답
        final_tool: 최종 도구 호출 정보 (JSON 내용)
    """
    try:
...
async def initialize_session(mcp_config=None):
    ...
    try:
        with st.spinner("🔄 MCP 서버에 연결 중..."):
            ...
            # Anthropic 모델 (Claude 3.7 Sonnet) 및 ReAct 에이전트 생성
            model = ChatAnthropic(model="claude-3-7-sonnet-latest", temperature=0.1, max_tokens=20000)
            agent = create_react_agent(
                model,
                tools,
                checkpointer=MemorySaver(),
                prompt="Use your tools to answer the question. Answer in Korean.",  # 프롬프트 문자열 한 줄로 수정
            )
...
with st.sidebar.expander("MCP 도구 추가", expanded=False):
  ...
  "weather": {
    ...
    # pending config가 없으면 기존 설정 텍스트에서 로드
    ...
    if st.button("도구 추가", type="primary", key="add_tool_button", use_container_width=True):
        try:
            ...
            if not new_tool_json.strip().startswith("{") or not new_tool_json.strip().endswith("}"):
                ...
                # mcpServers 키가 있으면 내부 내용으로 대체
                if "mcpServers" in parsed_tool:
                    parsed_tool = parsed_tool["mcpServers"]
                    st.info("'mcpServers' 형식이 감지되어 자동으로 변환되었습니다.")
                ...
                if len(parsed_tool) == 0:
                    ...
                    for tool_name, tool_config in parsed_tool.items():
                        ...
                        if "command" not in tool_config and "url" not in tool_config:
                            ...
                            st.error(f"'{tool_name}' 도구의 'args' 필드는 반드시 리스트 형식이어야 합니다.")
                        else:
                            # pending 설정에 도구 추가
                            st.session_state.pending_mcp_config[tool_name] = tool_config
                            success_tools.append(tool_name)

                    # 성공적으로 추가된 도구에 대한 피드백
                    if success_tools:
            ...
            **수정 방법**:  
            ...
            4. 문자열 내부에 큰따옴표를 포함해야 하면 이스케이프(\\") 처리하세요.
            """
            )
        except Exception as e:
            st.error(f"오류 발생: {e}")
...
with st.sidebar:
    # "적용하기" 버튼: pending 설정을 실제 적용하고 에이전트를 재초기화
...
if user_query:
    if st.session_state.session_initialized:
        ...
        with st.chat_message("assistant"):
            ...
            # run_until_complete로 비동기 답변 생성 호출
...
with st.sidebar:
    ...
    if st.button("🔄 대화 초기화", use_container_width=True, type="primary"):
        st.session_state.thread_id = random_uuid()   # thread_id 초기화
