...
st.sidebar.markdown("### 💡 MCP Agent (https://youtube.com/c/teddynote) 💡")
st.sidebar.divider()
st.title("💻 MCP Agent와 함께 멋진 세상이 열립니다 ")
st.markdown("🧊 MCP Agent와 대화해요 🧊.")
...
with st.sidebar.expander("MCP 추가", expanded=False):
...
with st.sidebar:
    if st.button("MCP 도구 적용", key="apply_button", type="primary", use_container_width=True):
